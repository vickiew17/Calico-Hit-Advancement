...
chr19p <- dplyr::select(armlevel, V1,`19p`,`15`)

viab.aneu.19paneu <- left_join(viab.aneu, chr19p, by = c("Cell_line" = "V1")) 
viab.aneu.19paneu %<>% dplyr::mutate(aneuploidy19p.num = ifelse(is.na(`19p`),NA,`19p`),aneuploidy19p.str = ifelse(is.na(`19p`),"NA",`19p`), logviability = log2(`2.5uM`/100), aneuploidy15.num = ifelse(is.na(`15`),NA,`15`),aneuploidy15.str = ifelse(is.na(`15`),"NA",`15`))
...
viab.aneu.19p.CERS1exp.noNA.formodel <- viab.aneu.19p.CERS1exp.noNA %>% dplyr::mutate_at(c("CERS1","aneuploidy19p.num"),funs(c(scale(.)))) 

viability.mod1 <- lm(logviability ~ CERS1, data = viab.aneu.19p.CERS1exp.noNA.formodel)
viability.mod2 <- lm(logviability ~ CERS1 + aneuploidy19p.num, data = viab.aneu.19p.CERS1exp.noNA.formodel)
...
```{r}
otherCERSexp.viab.aneu.19p.noNA <- otherCERSexp.viab.aneu.19p %>% dplyr::filter(!is.na(aneuploidy19p.num))

ggplot(otherCERSexp.viab.aneu.19p.noNA) + geom_point(aes(x = CERS1, y = CERS3,color = as.factor(aneuploidy19p.num)),size = 2) + theme(legend.position = "bottom")
```

```{r}
#mycomps <- combn(sort(unique(otherCERSexp.viab.aneu.19p.noNA$aneuploidy19p.str)),m = 2,simplify = F)
ggboxplot(data = otherCERSexp.viab.aneu.19p.noNA, x = "aneuploidy19p.str", y = "CERS3", order = sort(unique(otherCERSexp.viab.aneu.19p.noNA$aneuploidy19p.str)), title = "Baseline CERS3 expression by Chr19p aneuploidy") #+ stat_compare_means(comparisons = mycomps)
```
...
```{r}
otherCERSexp.viab.aneu.19p.noNA.formodel <- otherCERSexp.viab.aneu.19p.noNA %>% dplyr::mutate_at(c("CERS1","aneuploidy19p.num","CERS3"),funs(c(scale(.)))) 
#otherCERSexp.viab.aneu.19p15.noNA <- otherCERSexp.viab.aneu.19p %>% 
#    dplyr::filter(!is.na(aneuploidy19p.num),!is.na(aneuploidy15.num)) %>% dplyr::filter(aneuploidy15.num > -1)
viability.mod3 <- lm(logviability ~ CERS1 + aneuploidy19p.num, data = otherCERSexp.viab.aneu.19p.noNA)
viability.mod4 <- lm(logviability ~ CERS1 + aneuploidy19p.num + CERS3, data = otherCERSexp.viab.aneu.19p.noNA)
...
ggplot(pred) + geom_point(aes(x=logviability,y=ypred)) + geom_abline(aes(slope = 1, intercept = 0)) +# geom_smooth(aes(x=logviability,y=ypred),method = "lm") +
    labs(subtitle = paste0("R^2 = ",(1-mean(viability.mod4$residuals^2)/var(pred$logviability)))) #+ facet_grid(rows = vars(aneuploidy15.str),cols = vars(aneuploidy19p.str))
```
...
serexp.otherCERSexp.viab.aneu.19p.formodel <- serexp.otherCERSexp.viab.aneu.19p %>% dplyr::select(-lines,-CCLE_name,-`2.5uM`,-binnedjul,-binnedabs,-binnedgistic,-`19p`,-`15`,-aneuploidy19p.str, -aneuploidy15.str, -AUC) %>% scale %>% as.data.frame#, -jul,-abs,-gistic) #%>% dplyr::filter(!is.na(logviability))
...
To improve interpretability and observe interactions between the features, we derive a prediction rule ensemble using the "pre" R package. This model achieves an R^2 value of `r 1-(round(0.8866958,4)/round(var(d$logviability),4))`. The rules in the ensemble represented as decision trees are as follows:
...
```{r, eval = F}
#mycomps <- combn(sort(unique(otherCERSexp.viab.aneu.19p.noNA$aneuploidy15.str)),m = 2,simplify = F)
ggboxplot(data = otherCERSexp.viab.aneu.19p.noNA, x = "aneuploidy15.str", y = "CERS3", order = sort(unique(otherCERSexp.viab.aneu.19p.noNA$aneuploidy19p.str)), title = "Baseline CERS3 expression by Chr19p aneuploidy")# + stat_compare_means(comparisons = mycomps)
```
...
ggplot() + geom_point(aes(x=CERS1,y=CERS3,color = logviability < log2(.5)),size = 3,alpha = 0.5) + facet_wrap(aneuploidy19p.num~.)
